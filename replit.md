...
- **Delve Generation**: Procedurally generated 3-5 room dungeons with varied room types (Combat, Boss, Treasure, Puzzle, Trap) and tier-based difficulty. Players can abandon delves and return to their entrance location. Delves spawn at least 200 tiles away from Roboka. Completed delves are tracked and permanently removed from the map to prevent repetition.
...
- **Expanded Wilderness**: 3000x3000 world with camera-follow system (0.1 lerp) that smoothly tracks player movement. Eight procedurally placed delve entrances scattered across the map. Roboka city sprite (pixel art walled medieval city) visible in wilderness, provides instant return to town via 50px proximity trigger.
- **Terrain Generation**: Procedural terrain system using seeded random for deterministic world generation. Three terrain types: grass (85% with 3 color variants), dirt paths (10%), and pixel art trees (5%, 3 sprite variants). Delve exclusion zones prevent trees from spawning within 80px radius of delve entrances for clear access. Same coordinates always generate identical terrain. 32px tile size with viewport-optimized rendering.
...
- **Item Durability System**: All weapons and armor have durability that decays through use. Base durability is 100, increased by +10 per enhancement level. Weapons decay 1 point per combat, armor decays 1 point per combat plus 0.1 per tile moved (1 durability per 10 tiles). Items at 0 durability become unusable until repaired. Durability is displayed with color coding: green (>50%), yellow (26-50%), orange (11-25%), red (≤10% or broken). Players can repair items at the blacksmith with costs scaling by enhancement level (50 AA + 0.5 CA per level).
- **Forging & Enhancement System**: Blacksmith in Roboka provides +1 to +9 weapon/armor enhancement system with dual interface (Enhance/Repair tabs). Enhancement bonuses: odd levels (+1/+3) grant durability only; even levels (+2/+4/+6/+8) add +1 damage modifier; milestones (+5/+7/+9) add damage dice (weapons) or +2 evasion/+10% DR (armor). Success rates decrease from 95% (+1) to 10% (+9). Failure results in no change (tiers 1-2), downgrade (tiers 3+), or item destruction (tiers 5+, up to 50% at +9). Costs scale from 100 AA/0.1 CA to 5000 AA/5.0 CA. Enhanced items display "+X" suffix (e.g., "Steel Shortsword +5").
