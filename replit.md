...
### UI/UX Decisions
- **Visuals**: Pixel art for environments (city, wilderness, delve entrances, trees, bushes, grass), item sprites, player combat sprite, and enemy sprites (Void Spawn, Greater Void Spawn, Shadow Beast). NPCs are currently colored rectangles (120x120), except Blacksmith and Garthek the Stitcher which have custom circular shop button sprites (120x120) with character portraits. NPC grid uses 145px row spacing and 120px horizontal spacing for clean, uncluttered layout. Enemy sprites use 0.2 scale with tint effects for hover states.
...
- **Blacksmith's Forge Menu**: 900px wide panel with clean three-row header layout using generous spacing. Row 1 (Title): "Blacksmith's Forge" centered at y = -240. Row 2 (Currencies): AA and CA icons with values, centered at y = -175 with 30px horizontal gap between currency blocks and 57px spacing between icons and numbers (matching sidebar). Row 3 (Tabs): [Enhance], [Repair All], [Repair] buttons at y = -110. All header and item list text uses xsmall (14px) for complete visual consistency. 65px vertical gaps between header rows, 40px gap between tabs and item list. Tab spacing adapts: 220px for 3-button layout (with Repair All), 110px for 2-button layout (without). Item list uses fixed 3-column grid with 28px row height: Column 1 (item names) at width/2 - 420 (~420px width), Column 2 (enhancement/durability values) at width/2 (~150px width), Column 3 ([Select] buttons) at width/2 + 150.
