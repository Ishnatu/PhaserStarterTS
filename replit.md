...
- **Stats Panel**: Unified StatsPanel component (src/ui/StatsPanel.ts) displays player vitals in both Town and Wilderness scenes. Clean vertical layout with icon-based positioning: HP/SP bars (36px tall, no labels) with hover tooltips showing numeric values, vertically stacked currency (AA above CA) with 0.06 scale icons and 14px text, evasion icon + text (14px), DR icon + text (14px) directly underneath evasion, level text (14px) below DR. All elements maintain exactly 22px gaps between icon bottoms using dynamic spacing based on each icon's displayHeight. All stat text uses consistent 14px (xsmall) font at textX=47 for unified alignment. Panel is viewport-locked with scroll factor 0. Icons are user-provided pixel art.
