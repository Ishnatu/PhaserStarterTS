...
- **Stats Panel**: Unified StatsPanel component (src/ui/StatsPanel.ts) displays player vitals in both Town and Wilderness scenes. Features dark panel background (0x2a2a3e, 440x220px) with 15px internal padding. Clean layout: HP/SP bars (36px tall, no labels) with hover tooltips showing values, currency display with 0.06 scale icons and 30-42px spacing, level text (14px), evasion and DR icons/stats (0.06 scale, 14px text) with 47-52px label gaps. Panel is viewport-locked with scroll factor 0. HP/SP labels removed entirely - tooltips provide all information on hover. Icons are user-provided pixel art.
