...
- **Blacksmith's Forge Menu**: 900px wide panel with dynamic three-column layout using 20px spacing between sections. Both Enhancement and Repair tabs use runtime text-width measurements to position elements: item names at fixed startX (width/2 - 420), stats (enhancement level or durability) positioned at itemText.x + itemText.width + 20px, and [Select] buttons positioned at statsText.x + statsText.width + 20px. This ensures consistent 20px gaps regardless of item name length, preventing congestion while maintaining visual clarity.
