...
### UI/UX Decisions
- **Visuals**: Pixel art for environments (city, wilderness, delve entrances, trees, bushes, grass), item sprites, player combat sprite, and enemy sprites (Void Spawn, Greater Void Spawn, Shadow Beast). NPCs are currently colored rectangles (100x100), except Blacksmith and Garthek the Stitcher which have custom circular shop button sprites (100x100) with character portraits. Enemy sprites use 0.2 scale with tint effects for hover states.
- **Combat Backgrounds**: Two distinct pixel art backgrounds that automatically adapt based on encounter type. Delve combat uses an atmospheric void cavern with purple-tinted corrupted caves and glowing cyan crystals. Wilderness combat uses a vibrant forest clearing with trees, grass, and rocks. Both scale automatically to cover the full combat screen. Positioning differs by location: in delves, player on lower left platform (x=260, y=height-280) and enemies on upper center-right platform (x=740, y=200); in wilderness, player on left side of dirt path (x=380, y=height-360) and enemies on right side (x=730, y=height-300).
- **Combat Animations**: Player sprite has a lunge animation for attacks and a red hit flash when taking damage.
- **Typography**: Press Start 2P pixel font for all UI text and menus (improved readability over VT323). Font sizes increased by 25% for better visibility: small (20px), medium (25px), large (35px). All text uses `resolution: 2` for crisp pixel-perfect rendering.
...
- **Stats Panel**: Unified StatsPanel component (src/ui/StatsPanel.ts) displays player vitals in both Town and Wilderness scenes. Features dark panel background (0x2a2a3e, 420x200px) matching inventory/equipment style with layered borders. Clean horizontal layout: HP/SP bars (36px tall each) at top, currency display below using CurrencyDisplay component, level text, then evasion and damage reduction icons/stats side-by-side at bottom. Panel is viewport-locked with scroll factor 0 for persistent HUD display. All icons (currency, evasion, shield) use consistent 0.044 scale for visual harmony. Icons are user-provided pixel art.
