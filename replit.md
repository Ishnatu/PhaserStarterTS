...
- **Authentication**: Replit Auth (OpenID Connect) provides authentication via Google, GitHub, Apple, X, or email login - users don't need Replit accounts. Session-based auth using express-session with PostgreSQL session store. Players must authenticate before playing. Multi-instance detection prevents save conflicts. Game starts at MainMenuScene, validates authentication via /api/auth/me, displays username with logout option.
