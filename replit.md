...
- **Stats Panel**: Unified StatsPanel component (src/ui/StatsPanel.ts) displays player vitals in both Town and Wilderness scenes. Features dark panel background (0x2a2a3e) matching inventory/equipment style with layered borders. Clean layout: HP/SP bars with PixelArtBar at top, currency display below, then level and combat stats (Evasion with foot icon, Damage Reduction with shield icon) in organized rows. Panel is viewport-locked with scroll factor 0 for persistent HUD display.
