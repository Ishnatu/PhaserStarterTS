...
- **Blacksmith's Forge Menu**: 900px wide panel with clean three-row header layout using generous spacing. Row 1 (Title): "Blacksmith's Forge" centered at y = -240. Row 2 (Currencies): AA and CA icons with values, centered at y = -175 with 30px horizontal gap between currency blocks. Row 3 (Tabs): [Enhance], [Repair All], [Repair] buttons at y = -110. All header text uses xsmall (14px) for consistency with StatsPanel. 65px vertical gaps between rows provide breathing room. Tab spacing adapts: 220px for 3-button layout (with Repair All), 110px for 2-button layout (without). Item list uses fixed 3-column grid: Column 1 (item names) at width/2 - 420 (~420px width), Column 2 (enhancement/durability values) at width/2 (~150px width), Column 3 ([Select] buttons) at width/2 + 150.
