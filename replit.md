...
- **Stats Panel**: Unified StatsPanel component (src/ui/StatsPanel.ts) displays player vitals in both Town and Wilderness scenes. Features dark panel background (0x2a2a3e, 420x380px) matching inventory/equipment style with layered borders. Clean vertical layout stacked down the left side: HP bar (36px tall) → SP bar (36px tall) → AA currency → CA currency → Evasion stat → Damage Reduction stat → Level. Panel is viewport-locked with scroll factor 0 for persistent HUD display. All icons (currency, evasion, shield) use consistent 0.044 scale for visual harmony. Icons are user-provided pixel art.
