...
- **Typography**: Press Start 2P pixel font for all UI text and menus (improved readability over VT323). Font sizes: small (20px), medium (25px), large (35px), xlarge (45px). Main title uses large (35px), subtitle uses small (20px) for balanced hierarchy. All text uses `resolution: 2` for crisp pixel-perfect rendering.
...
- **Stats Panel**: Unified StatsPanel component (src/ui/StatsPanel.ts) displays player vitals in both Town and Wilderness scenes. Features dark panel background (0x2a2a3e, 440x220px) with 15px internal padding. Clean layout: HP/SP bars (36px tall) with hover tooltips showing numeric values, currency display with 0.06 scale icons and 30-42px spacing, level text, evasion and DR icons/stats (0.06 scale) with 47-52px label gaps. Panel is viewport-locked with scroll factor 0. HP/SP numeric values hidden by default, shown on hover via interactive tooltips. Icons are user-provided pixel art.
