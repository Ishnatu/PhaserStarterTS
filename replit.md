...
- **Blacksmith's Forge Menu**: 900px wide panel with clean three-row header layout. Row 1 (Title): "Blacksmith's Forge" centered at y = -240. Row 2 (Currencies): AA and CA icons with values, centered at y = -200 with 30px horizontal gap between currency blocks. Row 3 (Tabs): [Enhance], [Repair All], [Repair] buttons horizontally spaced 150px apart at y = -160, centered as a group. 40px vertical gaps between rows prevent all overlap. Item list uses fixed 3-column grid: Column 1 (item names) at width/2 - 420 (~420px width), Column 2 (enhancement/durability values) at width/2 (~150px width), Column 3 ([Select] buttons) at width/2 + 150.
