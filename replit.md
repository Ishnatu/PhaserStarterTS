...
- **Stats Panel**: Unified StatsPanel component (src/ui/StatsPanel.ts) displays player vitals in both Town and Wilderness scenes. Features dark panel background (0x2a2a3e, 440x220px) with 15px internal padding and thicker borders (4px outer, 2px inner) for prominent framing. Clean vertical layout: HP/SP bars (36px tall, no labels) with hover tooltips, vertically stacked currency (AA above CA) with 0.06 scale icons and 14px text, level text (14px), evasion icon + text (14px), DR icon + text (14px). All stat text uses consistent 14px (xsmall) font for unified appearance. Panel is viewport-locked with scroll factor 0. Icons are user-provided pixel art.
