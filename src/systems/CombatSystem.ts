...
export class CombatSystem {
  ...
  enemyTurn(): string[] {
    ...
    for (const enemy of aliveEnemies) {
      ...
      // Use CSV-driven attacks if available
      if (enemy.attacks && enemy.attacks.length > 0) {
        const attackLogs = this.executeEnemyAttack(enemy);
        logs.push(...attackLogs);
        continue;
      }

      // Fallback to legacy hardcoded behavior for backwards compatibility
  ...
  private executeEnemyAttack(enemy: Enemy): string[] {
    if (!this.combatState || !enemy.attacks) return [];
    
    // Filter available attacks based on triggers and usage limits
    const availableAttacks = enemy.attacks.filter(attack => 
      this.isAttackAvailable(enemy, attack)
    );

    if (availableAttacks.length === 0) {
      // Fallback to basic attack
      return this.executeBasicEnemyAttack(enemy);
    }

    // Select attack using weighted random
    const selectedAttack = this.selectWeightedAttack(availableAttacks);
    
    // Execute the attack
    return this.performEnemyAttack(enemy, selectedAttack);
  }

  private isAttackAvailable(enemy: Enemy, attack: any): boolean {
    // Check usage limits
    if (attack.maxUses !== undefined && enemy.attackStates) {
      const state = enemy.attackStates.get(attack.name);
      if (state && state.usesRemaining !== undefined && state.usesRemaining <= 0) {
        return false;
      }
    }

    // Check trigger conditions
    if (attack.trigger.type === 'always') {
      return true;
    } else if (attack.trigger.type === 'chance') {
      return Math.random() < attack.trigger.value;
    } else if (attack.trigger.type === 'health_threshold') {
      const healthPercent = enemy.health / enemy.maxHealth;
      return healthPercent < attack.trigger.value;
    }

    return true;
  }

  private selectWeightedAttack(attacks: any[]): any {
    const totalWeight = attacks.reduce((sum, attack) => sum + attack.priorityWeight, 0);
    let random = Math.random() * totalWeight;

    for (const attack of attacks) {
      random -= attack.priorityWeight;
      if (random <= 0) {
        return attack;
      }
    }
  ...
  private performEnemyAttack(enemy: Enemy, attack: any): string[] {
    if (!this.combatState) return [];
    
    const logs: string[] = [];

    // Handle special effect-only attacks (no damage)
    if (attack.damage.numDice === 0 && attack.damage.dieSize === 0) {
      const effectLogs = this.applyAttackEffect(enemy, attack);
      logs.push(...effectLogs);
      this.decrementAttackUses(enemy, attack);
      return logs;
    }
    ...
    if (!hit) {
      const missMessage = `${enemy.name} uses ${attack.name} and misses! (Rolled ${attackResult.d20}+3=${attackResult.total} vs Evasion ${playerEvasion})`;
      this.combatState.combatLog.push(missMessage);
      logs.push(missMessage);
      return logs;
    }
    ...
    if (attackResult.critical) {
      const critResult = DiceRoller.rollCriticalDamage(attack.damage);
      damageBeforeReduction = critResult.total;
      damageRollInfo = `CRITICAL HIT! (${critResult.maxDie} max + ${critResult.extraRoll} roll + ${critResult.modifier} = ${critResult.total})`;
    } else {
      const damageResult = DiceRoller.rollDiceTotal(attack.damage);
      damageBeforeReduction = damageResult.total;
      const rollsStr = damageResult.rolls.join('+');
      damageRollInfo = `(${rollsStr}+${damageResult.modifier} = ${damageResult.total})`;
    }
    ...
    let logMessage = `${enemy.name} uses ${attack.name}! ${damageRollInfo}`;
    if (totalDR > 0) {
      logMessage += ` -> ${damage} damage after ${Math.floor(totalDR * 100)}% reduction`;
    } else {
      logMessage += ` -> ${damage} damage`;
  ...
  private applyAttackEffect(enemy: Enemy, attack: any): string[] {
    if (!this.combatState) return [];
    
    const logs: string[] = [];

    if (attack.effect.type === 'apply_condition') {
      const params = attack.effect.params;
      const condition = params.condition;
      
      let duration = 1;
      if (params.duration_dice && params.duration_die) {
        duration = DiceRoller.rollDice(params.duration_dice, params.duration_die).total;
      } else if (params.duration) {
        duration = params.duration;
      }
      
      ConditionManager.applyCondition(this.combatState.player, condition, duration, 1);
      
      const message = `${enemy.name} uses ${attack.name}! ${attack.description} (${condition} for ${duration} rounds)`;
      this.combatState.combatLog.push(message);
      logs.push(message);
    } else if (attack.effect.type === 'heal_from_history') {
      const params = attack.effect.params;
      let lookbackRounds = 1;
      
      if (params.lookback_dice && params.lookback_die) {
        lookbackRounds = DiceRoller.rollDice(params.lookback_dice, params.lookback_die).total;
      } else if (params.lookback) {
      ...
      const usesRemaining = enemy.attackStates?.get(attack.name)?.usesRemaining ?? 'unlimited';
      const message = `${enemy.name} uses ${attack.name}! Time reverses ${lookbackRounds} rounds, healing ${totalHealing} HP! (${usesRemaining} uses remaining)`;
      this.combatState.combatLog.push(message);
      logs.push(message);
    }
  ...
  private decrementAttackUses(enemy: Enemy, attack: any): void {
    if (attack.maxUses !== undefined && enemy.attackStates) {
      const state = enemy.attackStates.get(attack.name);
      if (state && state.usesRemaining !== undefined) {
        state.usesRemaining--;
      }
    }
  }

  private executeBasicEnemyAttack(enemy: Enemy): string[] {
    if (!this.combatState) return [];
